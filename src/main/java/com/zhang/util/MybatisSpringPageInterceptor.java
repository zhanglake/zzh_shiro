...
public class MybatisSpringPageInterceptor implements Interceptor {
    ...
    protected String buildMysqlPageSql(Page<?> page, String sql) {
        ...
        // 附带join的关联分页
        if (null != page.getParams().get("limit_position")) {
            int index = 0;
            int length = (Integer) page.getParams().get("limit_position") + 1;
            for (int i = 0; i < length; i++) {
                index = sql.indexOf(")", ++index);
            }
            StringBuilder sb = new StringBuilder(sql);
            sb.insert(index, new StringBuilder(" limit ").append(offset).append(",").append(page.getPageSize()));
            return sb.toString();
        }
    ...
    protected void queryTotalRecord(Page<?> page, Object parameterObject, MappedStatement mappedStatement, Connection connection) throws SQLException {
        ...
        String sql = boundSql.getSql();
        String idName = page.getParams().get("idName").toString();
        String countSql = this.buildCountSql(idName, sql);
        if (log.isDebugEnabled()) {
     ...
     * @return
     */
    protected String buildCountSql(String idName, String sql) {
        int index = sql.indexOf("FROM");
        if (null != idName) {
            return "select count(distinct " + idName + ") " + sql.substring(index);
        }
